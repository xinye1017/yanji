...
internal fun findMarkupSpans(content: String): List<MarkupSpan> {
    ...
    scan(NoteMarkup.Underline, MarkupKind.Underline)
    scan(NoteMarkup.Bold, MarkupKind.Bold)
    scan(NoteMarkup.Italic, MarkupKind.Italic)

    return spans.sortedBy { it.openStart }
}

/** 该行是否为分隔线（至少 [NoteMarkup.DividerMinRun] 个破折号，允许前后空白）。 */
internal fun isDividerLine(line: String): Boolean {
    val trimmed = line.trim()
    if (trimmed.isEmpty()) return false
    return trimmed.all { it == '—' || it == '-' } && trimmed.length >= NoteMarkup.DividerMinRun
}
 ...
 */
fun noteMarkdownTransformation(): VisualTransformation = VisualTransformation { annotated ->
 ...
 */
fun stripNoteMarkup(content: String): String {
    ...
    return kept.lineSequence()
        .mapNotNull { raw ->
            when {
                ...
                else -> raw.trimEnd().removePrefix(NoteMarkup.Bullet)
            }
        }
        .joinToString("\n")
}
...
internal fun toggleInlineMarkup(
    ...
    val token = when (kind) {
        MarkupKind.Bold -> NoteMarkup.Bold
        MarkupKind.Italic -> NoteMarkup.Italic
        MarkupKind.Underline -> NoteMarkup.Underline
    }
...
internal fun isBulletLine(text: String, offset: Int): Boolean {
    ...
    return text.substring(lineStart, lineEnd).startsWith(NoteMarkup.Bullet)
}
