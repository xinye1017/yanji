...
import com.example.yanji.data.JuanjuanResponseBlock
...
object JuanjuanResponseParser {
     ...
     */
    fun parse(content: String): JuanjuanResponse {
        if (content.isBlank()) {
            return JuanjuanResponse()
        }
        val rawLines = content.lines().map { it.trim() }.filter { it.isNotEmpty() }
        if (rawLines.isEmpty()) return JuanjuanResponse()
    ...
    private fun structuredParse(lines: List<String>): JuanjuanResponse {
        var diagnosis: String? = null
        var evidence: String? = null
        val steps = mutableListOf<String>()
        val actions = mutableListOf<JuanjuanAction>()
        val followups = mutableListOf<String>()
        val mainBlocks = mutableListOf<JuanjuanResponseBlock>()
        ...
        for (raw in lines) {
            ...
            ASK_PLAN_LINE.matchEntire(raw)?.let {
                actions += JuanjuanAction(
                    id = UUID.randomUUID().toString(),
                    type = JuanjuanActionType.CREATE_PLAN,
            ...
            // 其余正文
            mainBlocks += JuanjuanResponseBlock(JuanjuanBlockKind.MAIN, stripBold(raw))
        }

        // 步骤汇总为一个块；followups 单独成块便于 FlowRow 渲染
        val blocks = buildList {
            if (steps.isNotEmpty()) {
                add(JuanjuanResponseBlock(JuanjuanBlockKind.STEPS, steps.joinToString("\n")))
            }
            addAll(mainBlocks)
        }
    ...
    private fun parseActionType(raw: String): JuanjuanActionType? = runCatching {
        JuanjuanActionType.valueOf(raw)
    }.getOrNull()

    /**
     * 旧版本的纯文本回退解析。
     * 行为与重构前的 parseJuanjuanMessage 保持一致：
     *   - 编号行 → steps
     ...
     *   - 其余 → paragraphs
     */
    internal fun fallbackParse(content: String): JuanjuanResponse {
        ...
        val blocks = buildList {
            calloutBox?.let { add(JuanjuanResponseBlock(JuanjuanBlockKind.MAIN, it)) }
            paragraphs.forEach { add(JuanjuanResponseBlock(JuanjuanBlockKind.MAIN, it)) }
            if (steps.isNotEmpty()) {
                add(JuanjuanResponseBlock(JuanjuanBlockKind.STEPS, steps.joinToString("\n")))
            }
        }
