...
class JournalGroupingTest {
    ...
    fun groupsByDateDescendingAndEntriesByCreatedAtDescending() {
        ...
        val state = JournalViewModel.groupsOf(entries)
    ...
    fun emptyInputProducesNoGroups() {
        val state = JournalViewModel.groupsOf(emptyList())
        assertTrue(state.groups.isEmpty())
        assertTrue(state.journals.isEmpty())
    }
    ...
    fun dayHeaderLabelRendersChineseMonthAndDay() {
        assertEquals("9月21日", journalDayHeaderLabel("2026-09-21"))
        assertEquals("1月1日", journalDayHeaderLabel("2026-01-01"))
        assertEquals("12月31日", journalDayHeaderLabel("2026-12-31"))
    }

    @Test
    fun dayHeaderLabelFallsBackToRawValueWhenUnparseable() {
        assertEquals("not-a-date", journalDayHeaderLabel("not-a-date"))
    }
    ...
    fun emptyQueryReturnsEverything() {
        ...
        assertEquals(entries, filterJournals(entries, ""))
        assertEquals(entries, filterJournals(entries, "   "))
    }

    @Test
    fun keywordMatchesTitleContentAndTags() {
        val target = JournalEntry(id = "x", date = "2026-09-21", title = "数学复盘", content = "中值定理失分")
        val tagged = JournalEntry(id = "y", date = "2026-09-20", content = "今天", tags = listOf("待办计划"))
        val other = JournalEntry(id = "z", date = "2026-09-19", content = "英语阅读")
        val all = listOf(target, tagged, other)

        assertEquals(listOf("x"), filterJournals(all, "定理").map { it.id })
        assertEquals(listOf("x"), filterJournals(all, "复盘").map { it.id })
        assertEquals(listOf("y"), filterJournals(all, "待办").map { it.id })
        assertTrue(filterJournals(all, "不存在的词").isEmpty())
    }

    @Test
    fun keywordMatchIsCaseInsensitive() {
        val e = JournalEntry(id = "x", date = "2026-09-21", content = "DeepSeek 分析")
        assertEquals(listOf("x"), filterJournals(listOf(e), "deepseek").map { it.id })
    }

    @Test
    fun dateQueryMatchesIsoAndChineseForms() {
        val a = JournalEntry(id = "a", date = "2026-09-21", content = "x")
        val b = JournalEntry(id = "b", date = "2026-09-20", content = "y")
        val all = listOf(a, b)

        // ISO 全写
        assertEquals(listOf("a"), filterJournals(all, "2026-09-21").map { it.id })
        // 月-日
        assertEquals(listOf("a"), filterJournals(all, "09-21").map { it.id })
        // 中文「9月21日」
        assertEquals(listOf("a"), filterJournals(all, "9月21日").map { it.id })
        // 斜杠
        assertEquals(listOf("a"), filterJournals(all, "9/21").map { it.id })
    }

    @Test
    fun searchThenGroupKeepsDayStructure() {
        val all = listOf(
            JournalEntry(id = "m1", date = "2026-09-21", content = "数学一"),
            JournalEntry(id = "m2", date = "2026-09-21", content = "数学二"),
            JournalEntry(id = "e1", date = "2026-09-21", content = "英语")
        )
        val filtered = filterJournals(all, "数学")
        val groups = JournalViewModel.groupsOf(filtered).groups
