...
```mermaid
graph TD
    UI[Compose Screens] --> VM[Feature ViewModels<br/>Home/Stats/Focus/Exam/Journal/<br/>Profile/Chat/CheckIn/AiConfig/Detail]
    VM --> REPO[YanjiRepository 单例]
    VM --> STATS[StudyStatisticsRepository]
    ...
    COORD -->|完成/放弃 落库| STORE
```

**分层现状**：
- UI → Feature ViewModel：全部主界面与子界面已接 ViewModel（第五轮完成），`UiState` 不可变，
  构造注入依赖，`repo: YanjiRepository = YanjiRepository.getInstance()` 默认参数模式已从 UI 层清除。
- ViewModel → Repository/StatsRepository：ViewModel 只做状态编排（combine/flatMapLatest）与动作转发。
- Repository → 领域 Store：状态与动作按域归属，Repository 保留同名委托以兼容。
...
1. UI 不应该负责决定「一次计时是否完成并落库」——已在计时链路上解决；
2. Repository 不应该同时是数据库缓存、AI Client、业务层和全局状态容器——Store 层已拆出
   （Timer/Chat/Journal/CheckIn/Preset 五个域），Repository 收敛为「数据库绑定 + 同名委托」；
   后续可继续把 ViewModel 直连 Store，绕过委托层。
...
| 打卡 / 连续天数 | ✅ Implemented | `data/checkin/CheckInStore` + `CheckInLogic` | `CheckInLogicTest`（10） |
| 快捷操作排序 | ✅ Implemented | `data/preset/QuickStartPresetStore` + `PresetOrderLogic` | `PresetOrderLogicTest`（7） |
| 学习时长口径 | ✅ Implemented | `data/study/StudyStats`（只统计 COMPLETED） | `StudyStatsTest`（6） |
| Feature ViewModel 层 | ✅ Implemented | `ui/**/*ViewModel`（10 个域，UiState 不可变 + 构造注入） | 编译期约束 + CI |
| 连续有效学习天数如实呈现 | ✅ Implemented | `StudyStatisticsRepository`（不再 `maxOf(1, streak)`） | 消费方按 0 分支处理 |
| 专注/模考生命周期 | ✅ Implemented | `data/timer/TimerStore` | 待补 |
...
1. ~~**清理历史假种子数据**~~ ✅ **已在真机上清除（2026-09-11）**
   ...
   - 遗留说明：成就的解锁记录是按假数据达成的，`unlocked_achievements` 没有"按真实数据重算"
     的机制，目前保持原样。
2. **拆 `YanjiRepository`（阶段三）** —— **主体完成**
   - 已完成：`data/ai/AiClient` + `AiProtocol`（网络传输 / 协议解析 / 错误文案）；
     `data/chat/ChatStore`（会话与消息的状态与动作）；`data/timer/TimerStore`、
     `data/journal/JournalStore`、`data/checkin/CheckInStore` + `CheckInLogic`、
     `data/preset/QuickStartPresetStore` + `PresetOrderLogic`、`data/study/StudyStats`。
     Repository 1700 → 896 行，剩余部分为数据库绑定、委托与 Chat 上下文组装。
   - **顺带修掉的 bug**：启动时"当前会话"只在为空时才选，旧数据里残留的 `session-initial`
     会让 UI 停在一个不存在的会话上。现在 `ChatStore.ensureCurrentSessionIsValid()`
     在启动和备份导入后都会校正。
   - **可选后续**：ViewModel 直连各 Store，绕过 Repository 委托层；单 module 内做包级边界即可拿到大部分收益。
3. ~~**给 Screen 建 Feature ViewModel**~~ ✅ **第五轮完成（2026-09-11）**
   - 每个界面暴露一个不可变 `UiState`，依赖构造注入，替换
     `repo: YanjiRepository = YanjiRepository.getInstance()` 的默认参数写法。
   - 新建 12 个 ViewModel：`HomeViewModel` / `StatsViewModel` / `FocusViewModel` / `ExamViewModel` /
     `JournalViewModel` / `ProfileViewModel` / `ChatViewModel` / `CheckInViewModel` / `AiConfigViewModel` /
     `DailyStudyDetailViewModel` / `SubjectStudyDetailViewModel`（按导航参数 key 化）/
     `FocusSessionDetailViewModel`。
   - 界面状态（统计页的 tab/图表模式/统计层级、科目明细的时间范围）一并上移进 UiState；
     派生计算（倒计时天数、模考均分、本期总学时）在 ViewModel 的 combine 链中完成。
   - **顺带修掉的伪造统计**：`StudyStatisticsRepository.buildWeeklyStudySummary` 的
     `streakDays = maxOf(1, streak)` → `streak`。0 天连续学习现在如实显示 0，
     HomeScreen 的"还没有连续学习记录"引导分支恢复可达。
   - ViewModel 挂 Activity scope（手写 screenStack 导航没有 per-destination store），
     跨 tab/子界面共享同一实例，与进程级 Store 语义一致。
4. **统计下沉到 DAO**
