...
| **`YanjiChip`** | Filter / Choice Capsule (`UIButton.Configuration.capsule()`) | **Compose**: `FilterChip` / `Surface` with height `36.dp` (padded to 48dp touch boundary), shape `PillRadius`. Selected: `primarySoft` fill + `primary` text. Unselected: `secondaryFill` fill + `secondaryLabel` text. | Compact metadata or filter tag; touch target padded via `minimumInteractiveComponentSize()`. |
...
| **`YanjiMetric`** | Metric Card (`WidgetKit` Number Display) | **Compose**: Prominent typography: Number rendered in `metricXl` (34sp Bold) or `metricL` (28sp Bold), label in `footnote` (13sp Medium). Employs `RollingNumber` animation for value changes. | High visual clarity for dashboard numbers (countdown days, study hours, streak counts). |
| **Progress Ring** | `UIProgressView` / Activity Ring | **Compose**: `Canvas` arc drawing with rounded caps, background track `secondaryFill`, progress stroke `primary` (or lavender accent gradient). Respects `reduceMotion` (instant transition instead of perpetual spin). | Clean metric visualization; avoids cluttering with ticks or percentage numbers inside narrow rings. |
