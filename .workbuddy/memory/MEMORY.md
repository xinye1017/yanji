...
- Kotlin daemon 偶发崩溃（`e: Daemon compilation failed`），Gradle 自动 fallback 且 BUILD SUCCESSFUL；看 `e:` 先确认末尾结果。多任务并发会加剧。
- 变量名**禁用 `TMP`/`TEMP`**（覆盖 Windows 临时目录，adb server 起不来）。
- 模拟器镜像被删残（缺 `system.img`/`vendor.img`）时 `sdkmanager --install` 不重下；必须把该目录移出 **SDK 根**再重下。AVD `DiaryPhone` = API 35 / x86_64 / google_apis，WHPX 可用。
- `git worktree remove` 会因 build 目录长路径失败（`Filename too long`），残留目录需 `Remove-Item -Recurse -Force`。

## UI Design Token

- 唯一来源 `app/src/main/java/com/example/yanji/theme/`（`Color/Radius/Spacing/Type/Theme.kt`），**不是** `ui/theme/`。
- **`ui/` 下禁止 `Color(0x...)` 字面量**，用 `MaterialTheme.colorScheme.*` / `YanjiColors.*`（静态亮色 token 暗色下永远是亮色值）。
...
- ❗**每个 `Modifier.hazeEffect { }` 块必须显式设置 `backgroundColor`**。haze 1.5.4 的 RenderEffect 路径
  硬性 `require(bg.isSpecified)`，三级解析全空即抛 `IllegalArgumentException("backgroundColor not specified...")`。
  这是启动期绘制崩溃，单测测不出，只真机暴露。
...
- 亮色 `DESIGN.md`，暗色仓库根 `design_dark.md`（Midnight Blue）：底 `#0D111A` / 卡 `#151B28` / 控件 `#1D2536` / 浮层 `#222C40`；
...
- `JournalViewModel.saveJournal` 是 `open`，留给插桩测试注入不落库子类。
- 插桩必须用一次性数据库，**绝不能打用户真库**。`LiveActivityCapabilityInstrumentedTest` 用 `assumeTrue/assumeFalse` 表达条件不变量。
...
## 并发改造风险（2026-09-14 与 2026-09-16 两次实测踩中）

- 同一仓库并发跑两个会话：另一进程会每 7~15 秒改写文件、**重置未提交的编辑**，还会把 HEAD 切到它自己的分支
  （2026-09-16 YANJI-A 期间被切走两次、2 个 commit 落错分支、1 处未提交编辑被重置）。
- ⇒ **大批量改动前先确认工作区归自己独占**；每个 commit 完成后**立即 push**，不要等任务收尾；
  commit 前必先 `git branch --show-current` 确认 HEAD；commit 落错分支时 cherry-pick 回自己的分支、
  用 `git branch -f <对方分支> <原 tip>` 还原对方指针；验证优先在 `git worktree add` 隔离副本里做。

## adb 运维

- **完整流程在仓库根 `AGENT.md` 第六节**（设备掉线重试、`scripts/dump_ui.py` 解析 uiautomator XML 取中心坐标、`cmd uimode night` 主题交叉验证）。
  真机 `PKB110` 逻辑分辨率 **1256×2760**。
...
- **验证「是否还崩」必须先 `adb logcat -c` 清空**；判定崩溃归属比对 crash log 最早时间戳与上次安装先后。
