...
- **陷阱**：曾见 Agent 每次改代码后都去跑复杂备份脚本、拉取 SQLite、用 `sqlite3` 检查 `user_version`、检查 `no_backup` 凭据——严重浪费时间且极易因 Windows 路径格式或环境缺失中断。
- **正道**：
  - **绝大多数日常 UI / 功能 / 业务逻辑修改**：只需编译 APK → 单次推送安装 → 启动查看效果。
...
10. **无证据不宣称通过**：严格区分「编译成功」「测试执行成功」「真机验证成功」；未运行项必须显式标注，环境故障不得伪装成产品缺陷。
...
判定标准：**状态栏底色与图标色必须互为对比色**；且 App 显式设为 LIGHT / DARK 时，结果**不应**随系统模式变化。两张截图字节完全一致，即是「不受系统影响」的有力证据。
...
3. **保留清单**：任何更新都不得删除 —— 核心架构原则、代理端口 `7898` 网络排查、mDNS 自动发现、ADB 同次调用铁律、真机交叉截图验证、提交 / 忽略纪律。
